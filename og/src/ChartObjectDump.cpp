...
// ChartObjectDump.cpp: ChartObject implementation for USER_GRAPH_DUMP (Mac/Linux).
// Writes graph data to JSON files instead of drawing with ZedGraph.
//////////////////////////////////////////////////////////////////////
...
void
ChartObject::DumpChartToFile(void)
{
	std::ostringstream fn;
	fn << "phreeqc_user_graph_" << this->n_user << ".json";
	std::string file_name = fn.str();
	std::ofstream f_out(file_name.c_str(), std::ios::out);
	if (!f_out.is_open())
	{
		std::ostringstream estream;
		estream << "Could not open file for USER_GRAPH dump: " << file_name;
		error_msg(estream.str().c_str(), CONTINUE);
		return;
	}
	f_out.precision(15);
	f_out << "{\n";
	f_out << "  \"chart_title\": \"" << json_escape(this->chart_title) << "\",\n";
	f_out << "  \"axis_titles\": [";
	for (size_t i = 0; i < this->axis_titles.size(); i++)
	{
		if (i > 0) f_out << ", ";
		f_out << "\"" << json_escape(this->axis_titles[i]) << "\"";
	}
	f_out << "],\n";
	auto write_scale = [&f_out](const LDBLE *scale, const char *key) {
		f_out << "  \"" << key << "\": [";
		for (int i = 0; i < 5; i++)
		{
			if (i > 0) f_out << ", ";
			if (i < 4 && (fabs(scale[i] - NA) < 1e-10))
				f_out << "\"auto\"";
			else if (i == 4 && fabs(scale[4] - 10.0) < 1e-10)
				f_out << "\"log\"";
			else if (i == 4)
				f_out << "null";
			else
	...
	write_scale(this->axis_scale_x, "axis_scale_x");
	write_scale(this->axis_scale_y, "axis_scale_y");
	write_scale(this->axis_scale_y2, "axis_scale_y2");
	f_out << "  \"plot_tsv_file\": [";
	for (size_t i = 0; i < this->csv_file_names.size(); i++)
	{
		if (i > 0) f_out << ", ";
		f_out << "\"" << json_escape(this->csv_file_names[i]) << "\"";
	}
	f_out << "],\n";
	f_out << "  \"series\": [\n";
	...
	for (i = 0; i < this->Curves.size(); i++)
		all_curves.push_back(this->Curves[i]);
	for (size_t c = 0; c < all_curves.size(); c++)
	{
		CurveObject *co = all_curves[c];
		if (c > 0) f_out << ",\n";
		std::string id = co->Get_id();
		if (id.empty())
			id = "Phase boundary";
		f_out << "    {\"id\": \"" << json_escape(id) << "\", "
		      << "\"color\": \"" << json_escape(co->Get_color()) << "\", "
		      << "\"symbol\": \"" << json_escape(co->Get_symbol()) << "\", "
		      << "\"y_axis\": " << co->Get_y_axis() << ", "
		      << "\"x\": [";
		const std::vector<LDBLE> &xv = co->Get_x();
		const std::vector<LDBLE> &yv = co->Get_y();
...
{
	/* Don't dump here; dump at End_timer() so JSON has all accumulated data */
	this->form_started = true;
	this->done = true;  /* no separate thread; allow End_timer to proceed */
	return true;
}
